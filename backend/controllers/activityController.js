...
const seed = [
  ...
  { type:'meditation',title:'10-Min Meditation',    description:'Full guided meditation for deep stress relief.',         forMoods:['stressed','angry','tired'],ageGroups:['young_adults'],                  icon:'🌿', duration:'10 min', route:'/meditation' },
...
export const getByMood = async (req, res) => {
  try {
    ...
    let list = await Activity.find({ forMoods: mood, ageGroups: ageGroup });
    if (!list.length) {
      await Activity.deleteMany({});
      await Activity.insertMany(seed);
      list = await Activity.find({ forMoods: mood, ageGroups: ageGroup });
    }
    res.json(list);
  } catch (err) {
    res.status(500).json({ message: err.message });
...
export const getByAgeGroup = async (req, res) => {
  try {
    const ageGroup = req.user?.ageGroup || 'young_adults';
    let list = await Activity.find({ ageGroups: ageGroup });
    if (!list.length) {
      await Activity.deleteMany({});
      await Activity.insertMany(seed);
      list = await Activity.find({ ageGroups: ageGroup });
    }
    res.json(list);
  } catch (err) {
    res.status(500).json({ message: err.message });
